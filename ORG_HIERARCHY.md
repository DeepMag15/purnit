...
- **Addendum (2026-08-10, Healthcare Domain, Phase A) — the first real proof that §1's "identical whether you're in Engineering, HR, or Sales" claim holds for a genuinely different industry, not just a hypothetical.** A second industry blueprint (`HEALTHCARE_BLUEPRINT_V1`, coexisting with the IT blueprint in the same `blueprints` table) defines 4 roles — Hospital Administrator, Doctor, Nurse, Receptionist — mapped onto this document's same universal tier ladder and 5-value scope enum, with **zero changes to this document's own model, `PermissionResolverService`, `isRowInScope`, or any `*Where()` scope-builder pattern**. Two new modules (Patients, Appointments) authorize through the identical mechanism §7's compliance rule already requires of every module — confirming the rule holds for a module this document's own authors never specifically had in mind when it was written, not just the ones named in §0's list above. **Department Types (§3/§4) are deliberately unpopulated for Healthcare in this phase** (`departmentTypes: []`, and the two new tables carry no `department_id` column at all) — a disclosed MVP narrowing, not a finding that the mechanism doesn't apply; every Healthcare grant uses only `own`/`tenant` scope as a result. **A real, disclosed nuance found during this work, worth recording here since it's about this document's own scope-check semantics, not Healthcare specifically**: `isRowInScope`'s `"own"` branch is an *ownership* match (a row's own `ownerId`/`assigneeId`-equivalent column), not a *membership* match — reusing an existing module's `own`-scoped permission for a *different* kind of "mine" relationship (e.g. "a project I'm a member of" rather than "a project I created") can silently deny access if the two don't happen to coincide. Full detail: `CONTEXT.md`, `CHANGELOG.md`'s matching session entry, `ARCHITECTURE.md`'s Healthcare Domain row (§9).
