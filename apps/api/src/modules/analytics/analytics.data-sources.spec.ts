...
describe("analytics.dashboard", () => {
  ...
  it("a zero-grant actor still receives an ungated metric", async () => {
    ...
    registry.register(gatedScalar("gated.metric", "settings:manage", 5));
    const dataSource = createAnalyticsDashboardDataSource(registry);
    const tx = { analyticsSnapshot: { findMany: jest.fn() } } as unknown as PrismaTx;
  ...
  it("populates trend at tenant scope and at department scope (with a real department) — absent (value still present) at own/team/department-subtree, or department scope with no department set", async () => {
    ...
    const txTenant = { analyticsSnapshot: { findMany: jest.fn().mockResolvedValue([{ value: 1 }, { value: 2 }]) } } as unknown as PrismaTx;
    ...
    const txDept = { analyticsSnapshot: { findMany: jest.fn().mockResolvedValue([{ value: 5 }]) } } as unknown as PrismaTx;
    ...
    const txDeptless = { analyticsSnapshot: { findMany: jest.fn() } } as unknown as PrismaTx;
    ...
    const txOwn = { analyticsSnapshot: { findMany: jest.fn() } } as unknown as PrismaTx;
